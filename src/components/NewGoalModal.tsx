...
export function NewGoalModal({ onClose, onSubmit, user }: NewGoalModalProps) {
  ...
  const totalAmount = useMemo(() => {
    // Multiplica por 200 para calcular o valor total dos depósitos
    return calculateTotalFromRange(maxNumber) * 200;
  }, [maxNumber]);
  ...
  const handleNumberChange = (value: string) => {
    ...
    if (calculationType === 'numbers') {
      ...
      if (amount > 0) {
        ...
        const numbers = calculateNumbersFromTotal(amount / 200);
